...
stop_epoch = 5000000000  # total games to be played
...
if __name__ == "__main__":
    ...
    model2.compile(optimizer=RMSprop(lr=learning_rate),loss='binary_crossentropy', metrics=['accuracy'])
    env = Env(grid_size)

    # Initialize experience replay object
    exp_replay_X = Agent(max_memory=max_memory)
    exp_replay_O = Agent(max_memory=max_memory)
    ...
    for e in range(stop_epoch):
        ...
        if e % display_epoch == display_epoch - 1:
            ...
            model1.save_weights("model/modelO.h5", overwrite=True)
            model2.save_weights("model/modelX.h5", overwrite=True)
            with open("model/modelO_full.json", "w") as outfile:
                json.dump(model1.to_json(), outfile)
            with open("model/modelX_full.json", "w") as outfile:
